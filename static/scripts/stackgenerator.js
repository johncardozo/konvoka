...
function updateStylesheet(iconSet) {
  // Generate a unique ID for this icon set's stylesheet
  var stylesheetId = 'dynamic-style-' + iconSet;
  var stylesheetUrl = styleSheets[iconSet];

  // Check if the stylesheet is already loaded
  var existingLink = document.getElementById(stylesheetId);
  if (!existingLink && stylesheetUrl) {
    // Stylesheet doesn’t exist and a valid URL is provided, so create and append it
    var link = document.createElement('link');
    link.id = stylesheetId;
    link.rel = 'stylesheet';
    link.href = stylesheetUrl;

    // Event handlers for load and error
    link.onload = function () {
      console.log(iconSet + ' stylesheet loaded');
    };
    link.onerror = function () {
      console.error('Failed to load ' + iconSet + ' stylesheet');
    };

    // Append to head
    document.head.appendChild(link);
  } else if (existingLink) {
    // Stylesheet already exists, no action needed
    console.log(iconSet + ' stylesheet is already loaded');
  } else {
    // No valid stylesheet URL for this iconSet, log a warning
    console.warn('No stylesheet defined for ' + iconSet);
  }
}
...
function findSuggestions(searchTerm) {
  ...
  if (semanticMatches) {
    return [...new Set(semanticMatches)].filter(icon => allIcons.includes(`-${icon}`) || allIcons.includes(icon)).map(icon => icon.replace(/^-/, '')); // Clean icons in results
  }
...
function filterIcons() {
  ...
  if (currentSearchTerm.length >= 2 && visibleIcons < 10) {
    ...
    suggestionsContainer.innerHTML = suggestions.length > 0 ? `<span class="suggest-title">Did you mean?</span> ${suggestions.map(s => `<span class="suggestion px-1" onclick="applySearch('${s}')">${s}</span>`).join(' ')}` : '';
  } else {
...
async function loadIconSet(iconSet = 'sa') {
  ...
  try {
    ...
    const iconSets = {
      'sa': {
        icons: 'json/sa-icons.json',
        mappings: 'json/sa-mappings.json',
        prefix: 'sa'
      },
      'base': {
        icons: 'json/sa-base.json',
        mappings: 'json/sa-mappings.json',
        prefix: 'sa'
      },
      'svg': {
        icons: 'json/sa-svg-icons.json',
        mappings: 'json/sa-svg-mappings.json',
        prefix: 'svg'
      },
      'fal': {
        icons: 'json/fa-icons.json',
        mappings: 'json/fa-mappings.json',
        prefix: 'fal'
      },
      'fas': {
        icons: 'json/fa-icons.json',
      ...
      'fab': {
        icons: 'json/fa-brands.json',
        mappings: 'json/fa-mappings.json',
        prefix: 'fab'
      },
      'fad': {
        icons: 'json/fa-duotone.json',
        mappings: 'json/fa-mappings.json',
        prefix: 'fad'
      },
      'material': {
        icons: 'json/material-icons.json',
        mappings: 'json/material-mappings.json',
        prefix: 'material'
      }
    };
    ...
    try {
      const [iconsResponse, mappingsResponse] = await Promise.all([fetch(selectedSet.icons), fetch(selectedSet.mappings)]);
...
function renderSearchHistory() {
  ...
  historyContainer.innerHTML = [...searchHistory].map(term => `<span class="badge bg-secondary me-1" onclick="applySearch('${term}')">
...
function updateVisibleCount() {
  ...
  document.querySelector('.results-count').textContent = `Showing ${visibleIcons} of ${totalIcons} icons`;
}
...
const getIconClass = (prefix, icon) => {
  ...
  switch (prefix) {
    ...
    case 'sa':
      return `<i class="sa sa-${cleanIcon}"></i>`;
    // SmartAdmin format: sa sa-iconname
    case 'base':
...
function copyToClipboard(text) {
  ...
  navigator.clipboard.writeText(copyText).then(() => console.log('Icon class/markup copied:', copyText)).catch(err => console.error('Failed to copy:', err));
}
...
function showToast(message, type = 'primary') {
  ...
  // Check if this is an icon class notification or a text message
  if (typeof type === 'string' && ['primary', 'secondary', 'success', 'danger', 'warning', 'info'].includes(type)) {
    ...
    toast.innerHTML = `
            <div class="toast-header bg-${type} text-white">
                <strong class="me-auto">Icon Stack Generator</strong>
    ...
    // This is an icon class notification (original functionality)
    const iconClass = message; // In this case, message is the icon class
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function initIconStackManagement() {
    ...
    function updateStackClasses() {
      ...
      iconLayers.forEach((layer, idx) => {
        ...
        layer.classList.remove('stack-1x', 'stack-2x', 'stack-3x');

        // Add appropriate stack class based on position
        if (idx === 0 && iconLayers.length >= 1) {
          layer.classList.add('stack-3x'); // Bottom layer (largest)
        } else if (idx === 1 || idx === 0 && iconLayers.length === 1) {
          layer.classList.add('stack-2x'); // Middle layer or single layer
        } else {
          layer.classList.add('stack-1x'); // Top layer (smallest)
        }
      });
    }
    ...
    function updateStackControlPanel() {
      ...
      iconLayers.forEach((layer, index) => {
            ...
            if (layerContent.includes('<svg') && layerContent.includes('href="')) {
              ...
              if (hrefMatch && hrefMatch[1]) {
                ...
                const filteredClasses = classes.filter(c => {
                  ...
                  return !['sa-icon', 'sa', 'stack-1x', 'stack-2x', 'stack-3x', 'fal', 'far', 'fas', 'fad', 'fab', 'text-primary', 'text-secondary', 'text-success', 'text-danger', 'text-warning', 'text-info', 'text-dark', 'text-light', 'sa-thin', 'sa-regular', 'sa-medium', 'sa-bold', 'sa-nofill', 'alpha-1', 'alpha-2', 'alpha-3', 'alpha-4', 'alpha-5', 'alpha-6', 'alpha-7', 'alpha-8', 'alpha-9', 'alpha-10'].includes(c) && !c.startsWith('rotate-');
                });
        ...
        // Extract current settings
        const currentSize = layer.classList.contains('stack-1x') ? '1x' : layer.classList.contains('stack-2x') ? '2x' : '3x';

        // Extract rotation class if any
        let currentRotation = '0';
        if (layer.classList.contains('rotate-45')) currentRotation = '45';else if (layer.classList.contains('rotate-90')) currentRotation = '90';else if (layer.classList.contains('rotate-135')) currentRotation = '135';else if (layer.classList.contains('rotate-180')) currentRotation = '180';else if (layer.classList.contains('rotate-225')) currentRotation = '225';else if (layer.classList.contains('rotate-270')) currentRotation = '270';else if (layer.classList.contains('rotate-315')) currentRotation = '315';

        // Extract opacity class if any
        let currentOpacity = '10';
        for (let i = 1; i <= 10; i++) {
          if (layer.classList.contains(`alpha-${i}`)) {
            currentOpacity = i.toString();
            break;
          }
        }
        ...
        if (isSvg) {
          ...
          if (svgElement) {
            if (svgElement.classList.contains('sa-thin')) currentSvgWeight = 'sa-thin';else if (svgElement.classList.contains('sa-regular')) currentSvgWeight = 'sa-regular';else if (svgElement.classList.contains('sa-medium')) currentSvgWeight = 'sa-medium';else if (svgElement.classList.contains('sa-bold')) currentSvgWeight = 'sa-bold';
        ...
        const layerHeader = `
                    <div class="d-flex flex-row border border-top-0 border-left-0 border-right-0 position-relative">
                        <div class="stack-icon-preview flex-shrink-0 d-flex align-items-center justify-content-center">
                            ${layerContent}
                        </div>
                        <div class="icon-settings d-flex flex-column flex-grow-1 border border-right-0 border-top-0 border-bottom-0 px-2 py-2 pe-4">
                            <div class="d-flex justify-content-between align-items-center flex-grow-1">
                                ...
                                <div class="layer-actions d-flex gap-1">
                                    ...
                                    <button class="btn btn-icon btn-outline-secondary edit-layer p-1 w-auto h-auto border-0" data-layer="${index}">
                                        <i class="sa sa-reload"></i>
                                    </button>
                                    <button class="btn btn-icon btn-outline-secondary tune-layer p-1 w-auto h-auto border-0 " data-layer="${index}" data-bs-toggle="collapse" data-bs-target="#layer-controls-${index}" aria-expanded="false">
                                        <i class="sa sa-settings"></i>
                                    </button>
                                    <button class="btn btn-icon btn-outline-danger delete-layer p-1 w-auto h-auto border-0" data-layer="${index}">
                        ...
                        <div class="control-panel-content shadow-inset-2 px-3 pt-1 pb-3" onclick="event.stopPropagation();">
                            ...
                            <div>
                                ...
                                <div class="d-flex flex-wrap gap-1">
                                    <button class="btn btn-icon btn-outline-default color-btn ${!currentColor ? 'active' : ''}" data-color="" data-layer="${index}">
                                        <i class="sa sa-ban"></i>
                                    </button>
                                    <button class="btn btn-icon btn-primary color-btn ${currentColor === 'primary' ? 'active' : ''}" data-color="primary" data-layer="${index}"></button>
                                    <button class="btn btn-icon btn-secondary color-btn ${currentColor === 'secondary' ? 'active' : ''}" data-color="secondary" data-layer="${index}"></button>
                                    <button class="btn btn-icon btn-success color-btn ${currentColor === 'success' ? 'active' : ''}" data-color="success" data-layer="${index}"></button>
                                    <button class="btn btn-icon btn-danger color-btn ${currentColor === 'danger' ? 'active' : ''}" data-color="danger" data-layer="${index}"></button>
                                    <button class="btn btn-icon btn-warning color-btn ${currentColor === 'warning' ? 'active' : ''}" data-color="warning" data-layer="${index}"></button>
                                    <button class="btn btn-icon btn-info color-btn ${currentColor === 'info' ? 'active' : ''}" data-color="info" data-layer="${index}"></button>
                                    <button class="btn btn-icon btn-dark color-btn ${currentColor === 'dark' ? 'active' : ''}" data-color="dark" data-layer="${index}"></button>
                                    <button class="btn btn-icon btn-default color-btn ${currentColor === 'light' ? 'active' : ''}" data-color="light" data-layer="${index}"></button>
                                </div>
                            </div>
                        </div>
                    </div>
      ...
      if (layerIndex >= 0 && layerIndex < iconLayers.length) {
        ...
        if (iconElement) {
          ...
          for (let i = 0; i <= 360; i += 45) {
            // Include 360 in the removal list
      ...
      if (layerIndex >= 0 && layerIndex < iconLayers.length) {
        ...
        if (isSvg) {
          ...
          if (svgElement) {
            ...
            const colorClasses = ['sa-icon-primary', 'sa-icon-secondary', 'sa-icon-success', 'sa-icon-danger', 'sa-icon-warning', 'sa-icon-info', 'sa-icon-dark', 'sa-icon-light'];
          ...
          if (iconElement) {
            // Remove existing color classes
            const colorClasses = ['text-primary', 'text-secondary', 'text-success', 'text-danger', 'text-warning', 'text-info', 'text-dark', 'text-light'];
            colorClasses.forEach(cls => {
    ...
    selectButton.addEventListener('click', function () {
      ...
      if (editingLayerIndex !== null) {
        ...
        if (editingLayerIndex >= 0 && editingLayerIndex < iconLayers.length) {
          iconLayers[editingLayerIndex].innerHTML = selectedIcon;
        }
        ...
        // Create new layer with appropriate stack class
        const newLayer = document.createElement('div');
        newLayer.className = 'icon-layers';

        // Add stack-Nx class based on layer position
        if (iconLayers.length === 0) {
          newLayer.classList.add('stack-3x'); // Bottom layer (largest)
        } else if (iconLayers.length === 1) {
          newLayer.classList.add('stack-2x'); // Middle layer
        } else {
          newLayer.classList.add('stack-1x'); // Top layer (smallest)
        }
        newLayer.innerHTML = selectedIcon;
    ...
    function deleteLayer(layerIndex) {
      ...
      if (layerIndex >= 0 && layerIndex < iconLayers.length) {
        ...
        if (confirm('Are you sure you want to delete this icon layer?')) {
          ...
          // Update stack classes for remaining layers
          const remainingLayers = iconContainer.querySelectorAll('.icon-layers');
          remainingLayers.forEach((layer, idx) => {
            // Remove all stack classes
            layer.classList.remove('stack-1x', 'stack-2x', 'stack-3x');

            // Add appropriate stack class based on new position
            if (idx === 0 && remainingLayers.length >= 1) {
              layer.classList.add('stack-3x'); // Bottom layer
            } else if (idx === 1 || idx === 0 && remainingLayers.length === 1) {
              layer.classList.add('stack-2x'); // Middle layer or single layer
            } else {
              layer.classList.add('stack-1x'); // Top layer
            }
          });
    ...
    document.getElementById('reset-layers').addEventListener('click', function () {
      if (confirm('Are you sure you want to reset all icon layers?')) {
        ...
        // Update the stack control panel
        updateStackControlPanel();
        showToast('All icon layers have been reset', 'info');
      }
    });
  }
  ...
  function copyIconToClipboard() {
    ...
    navigator.clipboard.writeText(iconHTML).then(() => showToast('Icon copied to clipboard!', 'success')).catch(() => showToast('Failed to copy icon', 'danger'));
  }
});
