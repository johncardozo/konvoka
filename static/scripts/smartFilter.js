...
(function (window) {
      ...
      for (var strIdx = 0; strIdx < str.length && patternIdx < pattern.length; strIdx++) {
        if (lowerPattern[patternIdx] === lowerStr[strIdx]) {
          ...
          if (lastMatchIdx === strIdx - 1) {
            consecutiveMatches++;
            score += CONSTANTS.MATCH_WEIGHTS.CONSECUTIVE + consecutiveMatches * CONSTANTS.MATCH_WEIGHTS.CONSECUTIVE_BONUS;
          } else {
  ...
  function ListFilter(list, input, options) {
    try {
      ...
      this.anchor = typeof this.settings.anchor === 'string' ? document.querySelector(this.settings.anchor) : this.settings.anchor;
  ...
  ListFilter.prototype = {
    _validateOptions: function (options) {
      ...
      try {
        ...
        if (options.maxLength !== undefined) {
          validated.maxLength = Math.min(CONSTANTS.MAX_INPUT_LENGTH, parseInt(options.maxLength, 10) || CONSTANTS.MAX_INPUT_LENGTH);
        }
    ...
    bindEvents: function () {
      try {
        ...
        this.input.addEventListener('input', this._validateEvent.bind(this), {
          passive: true
        });
        this.input.addEventListener('change', this._validateEvent.bind(this), {
          passive: true
        });
    ...
    _performFilter: function (filter) {
      try {
        ...
        // Update message
        if (this._cache.messageElement) {
          this._cache.messageElement.innerHTML = '<i class="fad fa-eye me-1"></i> ' + matchCount;
