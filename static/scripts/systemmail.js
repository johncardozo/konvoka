...
function resetComposeForm(clearAll = false) {
  ...
  document.getElementById('fake_textarea').innerHTML = `
    ...
    <div class="d-flex d-column align-items-start mb-3 gap-2">
        <img src="img/avatar-admin.png" alt="SmartAdmin WebApp" class="mr-3 mt-1 rounded-circle width-2">
...
function setupAttachments() {
  ...
  hiddenAttachments.forEach((attachment, index) => {
    ...
    attachmentEl.innerHTML = `${attachment.name} <button data-bs-dismiss="alert" class="btn btn-icon btn-xs ms-1 rounded-0 border border-${attachment.type} border-top-0 border-bottom-0 border-end-0 waves-effect waves-themed">
...
function loadEmails() {
  fetch('./json/MOCK_MAIL.json').then(response => {
    if (!response.ok) {
